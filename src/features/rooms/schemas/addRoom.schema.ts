...
export const createAddRoomSchema = (t: TFunction) =>
	z
		.object({
			name: z
		...
		.superRefine((data, context) => {
			if (data.minAge > data.maxAge) {
				...
				context.addIssue({
					code: z.ZodIssueCode.custom,
					message: t("rooms.validation.maxAgeLessThanMin"),
					path: ["maxAge"],
				})
			}
			if (data.studentIds.length > data.capacity) {
				context.addIssue({
					code: z.ZodIssueCode.custom,
					message: t("rooms.validation.studentCapacityExceeded", { capacity: data.capacity }),
					path: ["studentIds"],
				})
			}
		})

export type AddRoomFormData = z.infer<ReturnType<typeof createAddRoomSchema>>
