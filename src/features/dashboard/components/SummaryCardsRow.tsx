	...
	<Card className={`${heroCardClassName} min-h-[190px] p-0`}>
		<div className="grid h-full grid-cols-3 gap-4 p-6">
			{summaryMetrics.map((metric) => {
				...
				return (
					<div className="flex flex-col items-center text-center" key={metric.label}>
						<div className="flex size-10 items-center justify-center rounded-full bg-[#0485f7] text-white">
							<Icon className="size-5" />
						</div>
						<p className="mt-5 font-medium text-[#71717a] text-[12px]">{metric.label}</p>
			...
			<div className="grid grid-cols-2 gap-8">
				{signedInMetrics.map((metric) => (
					<div key={metric.label}>
						<p className="font-medium text-[#71717a] text-[12px]">{metric.label}</p>
						<div className="mt-2 flex items-end gap-3">
							<p className="font-semibold text-[#18181b] text-[40px] leading-none">{metric.value}</p>
							<p className="pb-1 font-medium text-[#71717a] text-[12px]">of {metric.total}</p>
						</div>
						<ProgressBar
							aria-label={`${metric.label} signed in`}
		...
		<div className="flex h-full flex-col gap-10 p-6">
			...
			<div className="grid grid-cols-2 gap-8">
				{absenteeGroups.map((group) => (
					<div key={group.label}>
						<p className="font-medium text-[#71717a] text-[12px]">{group.label}</p>
						<div className="mt-3">
