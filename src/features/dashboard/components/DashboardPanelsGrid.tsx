...
const RoomsStatusCard = () => (
	<Card className={`${panelCardClassName} col-span-1 p-0 lg:col-span-2`}>
		<div className="p-6">
			<h2 className="font-semibold text-[#18181b] text-[16px]">Rooms status today</h2>
...
const UpcomingCard = () => (
	<Card className={`${panelCardClassName} p-0`}>
		<div className="flex h-full min-h-[220px] flex-col p-6">
			<h2 className="font-semibold text-[#18181b] text-[16px]">Upcoming this week</h2>
			...
			<div className="mt-auto flex justify-end">
				<DashboardLink to="/calendar">View calendar</DashboardLink>
			</div>
		</div>
	</Card>
)
...
const AttentionCard = () => (
	<Card className={`${panelCardClassName} p-0`}>
		<div className="flex h-full min-h-[190px] flex-col p-6">
			<h2 className="font-semibold text-[#18181b] text-[16px]">Needs attention</h2>
			...
			<div className="mt-auto flex justify-end">
				<DashboardLink to="/reports">View more</DashboardLink>
			</div>
		</div>
	</Card>
)
...
const FinanceCardBlock = ({ card }: { card: (typeof financeCards)[number] }) => (
	<Card className={`${panelCardClassName} p-0`}>
		<div className="grid h-full min-h-[190px] grid-rows-[auto_1fr_auto] p-6">
			<h2 className="font-semibold text-[#18181b] text-[14px]">{card.label}</h2>
			<div className="flex items-center justify-center">
				<div>
					<p className={`text-center font-semibold text-[22px] sm:text-[40px] ${card.accentClassName}`}>{card.value}</p>
					<p className="mt-1 text-center text-[#71717a] text-[12px]">
						{card.label === "Payments" ? "Received" : "Total outstanding"}
					</p>
				</div>
			</div>
			<div className="flex justify-end">
				<DashboardLink to="/billing">{card.actionLabel}</DashboardLink>
			</div>
		</div>
	</Card>
)
	...
	return (
		<Card className={`${panelCardClassName} col-span-1 p-0 lg:col-span-2`}>
			<div className="min-h-[420px] p-6">
				<h2 className="font-semibold text-[#18181b] text-[16px]">Today&apos;s logged activities</h2>
										...
										{room.entries?.map((entry) => {
											...
											return (
												<div className="grid grid-cols-[20px_72px_1fr_24px] items-center gap-3" key={entry.label}>
													...
													<ProgressBar aria-label={`${entry.label} progress`} className="w-full" value={entry.progress}>
...
const DashboardPanelsGrid = () => (
	<div className="mt-6 grid gap-4 lg:grid-cols-3">
		...
		{financeCards.map((card) => (
			<FinanceCardBlock card={card} key={card.label} />
		))}
		<AttentionCard />
		<ActivitiesCard />
	</div>
