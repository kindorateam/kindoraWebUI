...
import { closeDrawerAtom, drawerIsOpenAtom, drawerOptionsAtom, resetDrawerAtom } from "@/stores/drawer.store"
...
const resolveTabKey = (key?: string | null): AddActivityTabKey => {
	...
	return (ADD_ACTIVITY_TABS.some((tab) => tab.key === key) ? key : DEFAULT_TAB) as AddActivityTabKey
}
...
const AppDrawer = () => {
	...
	const activeTabConfig = ADD_ACTIVITY_TABS.find((tab) => tab.key === activeTab) ?? ADD_ACTIVITY_TABS[0]
						...
						<DrawerHeader className="border-black/5 border-b pb-6">
							<div className="flex flex-col gap-6">
								...
								<Tabs
									...
									classNames={{
										base: "w-full",
										tabList: "flex gap-4 border-b border-black/10 bg-transparent px-0 pb-0",
									...
									onSelectionChange={(key) => setActiveTab(resolveTabKey(typeof key === "string" ? key : String(key)))}
						...
						<DrawerBody className="flex-1 overflow-y-auto py-6">
							<div className="flex flex-col gap-3">
								...
								<Text color="neutral-600" size={14}>
									...
									tab will live here. Use this space to outline fields, upload areas, and any helper text teams might
									need.
								</Text>
								<Text color="neutral-600" size={14}>
									Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras sit amet sem et lectus luctus vulputate.
