...
When working in this codebase, always prefer the built-in capabilities of the existing libraries before creating custom abstractions or custom UI.

**Required behavior:**
- Do **not** create custom components, wrappers, hooks, stores, utilities, fallback logic, or alternative implementations unless the user explicitly asks for a custom solution.
- First exhaust what the existing libraries already provide:
  - HeroUI
  - TanStack Router
  - TanStack Query
  - Jotai
  - Axios
  - Tiptap
  - other installed libraries already used in the repo
- If a library can handle the task, use the library directly.
- If a library might handle the task but support is unclear, check the local installed package/API surface first.
- If the library does not support the needed behavior cleanly, tell the user instead of inventing a custom solution.
- Do **not** build wrappers around library components or APIs unless the user explicitly requests that abstraction.

**Examples:**
- Use HeroUI `Avatar` or `AvatarGroup` instead of building custom avatar or fallback components.
- Use TanStack Router features before adding custom navigation state logic.
- Use TanStack Query patterns before creating custom async state management.
- Use Jotai atoms/utilities before custom state containers.
- Use Axios interceptors/config before custom request layers.
- Use Tiptap extensions/config before custom editor behavior.
...
## Commit & PR rules:
- Don't include internal comments
